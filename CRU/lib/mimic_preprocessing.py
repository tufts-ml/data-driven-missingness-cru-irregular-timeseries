...
#     data_df['minutes_from_admission'] = data_df['event_time_from_admit'].apply(get_minutes_from_adm)  
    ...
    # round the times up to 6 minutes as done by rubanova
    quantization=0.1
    data_df['hours_from_admission'] = round(data_df['hours_from_admission'] / quantization) * quantization
    ...
    print('Transforming data into %s minute buckets'%dt)
    for ii in pbar(range(nrows)): 
#         curr_t_hrs = data_df.iloc[fps[ii]:fps[ii+1]]['hours_from_admission'].values 
        curr_t_mins = data_df.iloc[fps[ii]:fps[ii+1]]['hours_from_admission'].values 
        curr_vals = data_df.iloc[fps[ii]:fps[ii+1]]['valuenum'].values
        t_start = 0
        t_end = np.ceil(curr_t_mins.max())
        if t_end==t_start:
            t_end=t_start+1e-5
        
        Tnew = np.unique(curr_t_mins) 
#         Tnew = np.arange(t_start, t_end, dt)
        Xnew = np.nan*np.ones_like(Tnew)
        if len(curr_vals)==1: 
            Xnew[-1] = curr_vals 
        else: 
            curr_bins = np.digitize(curr_t_mins, Tnew )-1
            Xnew[curr_bins] = curr_vals
#             F = interp1d(curr_t_mins, curr_vals, kind='previous', bounds_error=False, fill_value=np.nan)   
#             Xnew = F(Tnew) 
        labels_new = [data_df.iloc[fps[ii]]['label']]*len(Tnew) 
        stay_id_new = [data_df.iloc[fps[ii]]['stay_id']]*len(Tnew) 
        vals_list.append(Xnew) 
        t_list_mins.append(Tnew) 
        labels_list.append(labels_new) 
        stay_id_list.append(stay_id_new) 
        
    del data_df
    data_df = pd.DataFrame({'stay_id' : np.hstack(stay_id_list), 
                            'hours_from_admission':np.hstack(t_list_mins), 
...
#     feature_cols = keep_columns + ['bmi']
    ...
    final_df['timestamp'] = pd.to_datetime(final_df['admission_timestamp'])+pd.to_timedelta(final_df.hours_from_admission, unit='hours') 
