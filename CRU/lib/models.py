...
#     elif args.dataset == 'physionet':
#         model = MIMIC(target_dim=41, lsd=args.latent_state_dim, args=args,
#                 use_cuda_if_available=True)
    
    elif args.dataset == 'toy_mnar':
        model = ToyMnar(target_dim=2, lsd=args.latent_state_dim, args=args,
                use_cuda_if_available=True)
    
    elif args.dataset == 'mimic_4_extrap':
        model = MIMIC4Extrap(target_dim=2, lsd=args.latent_state_dim, args=args,
                use_cuda_if_available=True)
...
class MIMIC(CRU):
    ...
    def __init__(self, target_dim: int, lsd: int, args,
        ...
        super(MIMIC, self).__init__(target_dim, lsd,
                                             args, use_cuda_if_available, bernoulli_output)
...
class ToyMnar(CRU):
    ...
    def __init__(self, target_dim: int, lsd: int, args,
                 use_cuda_if_available: bool = True, bernoulli_output=False):
        self.hidden_units = args.hidden_units
        self.target_dim = target_dim

        super(ToyMnar, self).__init__(target_dim, lsd,
                                             args, use_cuda_if_available, bernoulli_output)
...
class MIMIC4Extrap(CRU):
    ...
    def __init__(self, target_dim: int, lsd: int, args,
        ...
        super(MIMIC4Extrap, self).__init__(target_dim, lsd,
                                             args, use_cuda_if_available, bernoulli_output)
