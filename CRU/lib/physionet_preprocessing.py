...
def normalize_data_and_save(input_path, name):
    ...
    norm_df = pd.DataFrame(np.column_stack([params_names, min_value, max_value]), columns=['feature', 'min', 'max'])
    return data_normalized, norm_df
...
def download_and_process_physionet(file_path):
    ...
    for set in sets:
        ...
        data_normalized, norm_df = normalize_data_and_save(processed_path, name=f'f37_{set}')
        torch.save(data_normalized, os.path.join(file_path, f'norm_{set}.pt'))
        norm_df.to_csv(os.path.join(file_path, 'normalization_estimates_%s.csv'%set), index=False)
    
    print('Pre-processed data saved to %s'%file_path)
if __name__ == '__main__':
    file_path = '/cluster/tufts/hugheslab/prath01/projects/time_series_prediction/datasets/'
