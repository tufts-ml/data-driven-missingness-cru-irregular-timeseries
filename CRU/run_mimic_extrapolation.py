...
# data configs
parser.add_argument('--dataset', type=str, default='mimic', help="Dataset to use. Available datasets are physionet, ushcn and pendulum.")
...
if __name__ == '__main__':
    ...
    train_dl = data.DataLoader(train_dataset, batch_size=128, shuffle=True, num_workers=2, drop_last=True)
    valid_dl = data.DataLoader(valid_dataset, batch_size=len(valid_X_NTD), shuffle=True, num_workers=2, drop_last=True)
    test_dl = data.DataLoader(test_dataset, batch_size=len(test_X_NTD), shuffle=True, num_workers=2, drop_last=True)
