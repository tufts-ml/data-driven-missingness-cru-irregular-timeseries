...
def evaluate(dim, rec, dec, test_loader, args, num_sample=10, device="cpu", kl_coef=1, supervised=False, eval_mask=None):
    mse, test_n, recon_loss = 0.0, 0.0, 0.0
    with torch.no_grad():
        for test_batch in test_loader:
            if supervised:
                test_batch, test_label = test_batch[0].to(device), test_batch[1].to(device)
            else:
            ...
            mse += mean_squared_error(observed_data, pred_x, eval_mask[:batch]) * batch
            test_n += batch
            
            recon_loss += recon_loss.item() * batch
            
            
    return mse / test_n, pred_x
...
def get_mimiciii_data(args):
    ...
    test_dataloader = DataLoader(
        test_data_combined, batch_size=len(test_data_combined), shuffle=False)
    val_dataloader = DataLoader(
        val_data_combined, batch_size=len(val_data_combined), shuffle=False)
...
def get_physionet_data(args, device, q, flag=1, full_batch=False):
    ...
    if flag:
        ...
        if full_batch:
            train_batch_size=len(train_data_combined)
            test_batch_size=len(test_data_combined)
        else:
            train_batch_size=batch_size
            test_batch_size=batch_size
        
        train_dataloader = DataLoader(
            train_data_combined, batch_size=train_batch_size, shuffle=False)
        test_dataloader = DataLoader(
            test_data_combined, batch_size=test_batch_size, shuffle=False)
        ...
        if classify:
            if activity:
                ...
                try:
                    ...
                    combined_labels[b] = torch.tensor(labels, dtype=torch.float32)
...
def irregularly_sampled_data_gen_square_sine(n=1000, length=50, seed=0):
    np.random.seed(seed)
    # obs_times = obs_times_gen(n)
    P=25
    D=10
    ...
    for i in range(n):
        t1 = np.linspace(0, 2, length)
        t2 = np.linspace(0, 2, length)
        f1 = np.arange(length) % P < D
        mask_0 = f1<.5
        mask_1 = f1>.5
        f2 = mask_1*np.sin(20*t2)+mask_0*np.cos(20*t2)+0.01*np.random.randn(len(t2))
        obs_times.append(np.stack((t1, t2), axis=0))
        obs_values.append(np.stack((f1, f2), axis=0))
        #obs_values.append([f1.tolist(), f2.tolist(), f3.tolist()])
        fg1 = np.arange(length) % P < D
        fg2 = mask_1*np.sin(20*t2)+mask_0*np.cos(20*t2)
        #ground_truth.append([f1.tolist(), f2.tolist(), f3.tolist()])
        ...
        mnar_inds_1 = np.logical_or(f1<-.3, f1>.3)
        miss_1 = np.asarray(Bernoulli(torch.tensor(0.2)).sample(f1.shape))
        ...
        mnar_inds_2 = np.logical_or(f2<-.3, f2>.3)
        miss_2 = np.asarray(Bernoulli(torch.tensor(0.2)).sample(f2.shape))
...
'''
def irregularly_sampled_data_gen_square_sine_2(n=1000, length=50, seed=0):
    np.random.seed(seed)
    # obs_times = obs_times_gen(n)
    P=25
    D=12
...
def get_toy_data_mnar_square_sine(args, N=1000):
    dim = 2
    n=N
    length=50
    ...
    for i in range(dim):
        combined_obs_values[:, i, i *
                            length: (i+1)*length] = obs_values[:, i]

        mask[:, i, i*length: (i+1)*length] = mask_vals[:, i]
    #print(combined_obs_values.shape, mask.shape, obs_times.shape, np.expand_dims(obs_times, axis=1).shape)
    combined_data = np.concatenate(
        (combined_obs_values, mask, np.expand_dims(obs_times, axis=1)), axis=1)
    combined_data = np.transpose(combined_data, (0, 2, 1))
...
def get_toy_data_mnar(args):
    ...
    b = 5
    N = 1000
    b_N = 0.01*np.random.randn(N)+b

    y_NT = np.zeros((N, T))
    for n in range(N):
        y_NT[n] = np.sin(12*(t+b_N[n]))


    t_NT = np.repeat(t[np.newaxis, :], N, axis=0)
    var = 0.01**2
    z_NT = np.sqrt(var)*np.random.randn(y_NT.shape[0], y_NT.shape[1])
    f_NT = z_NT+y_NT
    m_NT = np.asarray(Bernoulli(torch.tensor(0.6)).sample(f_NT.shape)) ## Get some irregularly sampled time points
    mnar_inds = np.logical_or(f_NT<-.3, f_NT>.3)
    miss_NT = np.asarray(Bernoulli(torch.tensor(0.2)).sample(f_NT.shape))
    m_NT[mnar_inds] = miss_NT[mnar_inds]
    ...
    print(train_data.shape, test_data.shape)
    train_dataloader = DataLoader(torch.from_numpy(
        train_data).float(), batch_size=batch_size, shuffle=False)
    test_dataloader = DataLoader(torch.from_numpy(
        test_data).float(), batch_size=batch_size, shuffle=False)
    data_obj = {"dataset_obj": combined_data,
                    "train_dataloader": train_dataloader,
                    "test_dataloader": test_dataloader,
                    "input_dim": 1,
                    "ground_truth": np.array(ground_truth)}
    return data_obj
'''
def get_toy_data_mnar(args, N=1000):
    seed=0
    np.random.seed(seed)
    T = 50
    t = np.linspace(0, 2, T)
...
#     b_N = 0.01*np.random.randn(N)+b
    ...
    b_N = np.zeros(N)
    for n in range(N):
        b_N[n] = np.random.uniform(0, 10, 1)+0.01*np.random.randn(1)
        y_NT[n] = np.sin(12*(t+b_N[n]))
